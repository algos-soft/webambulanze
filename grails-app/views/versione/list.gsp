...
<div id="list-versione" class="content scaffold-list" role="main">
    <h1><g:message code="versione.list.label" default="Elenco versione"/></h1>
    ...
    <table>
        ...
        <g:each in="${versioneInstanceList}" status="i" var="versioneInstance">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                <amb:rigaLista campiLista="${campiLista}" rec="${versioneInstance}" campiExtra="${campiExtra}"></amb:rigaLista>
            </tr>
        </g:each>
        </tbody>
    </table>

    <g:if test="${versioneInstanceTotal}">
